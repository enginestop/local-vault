...
export const api = {
  status: () => request<{ setup_completed: boolean; locked: boolean; vault_revision: number; api_version: string }>('GET', '/api/v1/status'),
  ...
  lock: () => request<{ locked: boolean }>('POST', '/api/v1/sessions/lock'),
  lockAll: () => request<{ locked: boolean }>('POST', '/api/v1/sessions/lock-all'),
  ...
  updateCredential: (id: string, body: Record<string, unknown>, revision: number) => request<Credential>('PUT', `/api/v1/credentials/${id}`, { body, headers: { 'If-Match': String(revision) } }),
  trashCredential: (id: string, revision: number) => request('POST', `/api/v1/credentials/${id}/trash`, { headers: { 'If-Match': String(revision) } }),
  restoreCredential: (id: string, revision: number) => request('POST', `/api/v1/credentials/${id}/restore`, { headers: { 'If-Match': String(revision) } }),
  purgeCredential: (id: string, revision: number) => request('DELETE', `/api/v1/credentials/${id}`, { headers: { 'If-Match': String(revision) } }),
  ...
  updateCategory: (id: string, name: string, revision: number) => request<Category>('PUT', `/api/v1/categories/${id}`, { body: { name }, headers: { 'If-Match': String(revision) } }),
  deleteCategory: (id: string, revision: number) => request('DELETE', `/api/v1/categories/${id}`, { headers: { 'If-Match': String(revision) } }),
  ...
  recoveryAction: (action: 'enable' | 'rotate' | 'disable', current_master_password: string) => request<{ recovery_key: string | null; enabled: boolean }>('POST', '/api/v1/settings/security/recovery-key', { body: { action, current_master_password } }),
